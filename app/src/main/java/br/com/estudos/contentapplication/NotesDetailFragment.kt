...
import br.com.estudos.contentapplication.database.NotesDatabaseHelper.Companion.DESCRIPTION_NOTES
...
import br.com.estudos.contentapplication.databinding.NoteDetailBinding
...
class NotesDetailFragment : DialogFragment(), DialogInterface.OnClickListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val view = activity?.layoutInflater?.inflate(R.layout.note_detail, null)
         ...
         editDescription = binding.editDescricao
         editTitle = binding.editTitulo
        ...
        if (arguments != null && arguments?.getLong(EXTRA_ID) != 0L){
            ...
            if (cursor?.moveToNext() as Boolean){
                newNote = false
                editTitle.setText(cursor.getString(cursor.getColumnIndex(TITLE_NOTES)))
                editDescription.setText(cursor.getString(cursor.getColumnIndex(DESCRIPTION_NOTES)))
            }
            cursor.close()
        }
    ...
    override fun onClick(dialog: DialogInterface?, which: Int) {
        ...
        values.put(TITLE_NOTES, editTitle.text.toString())
        values.put(DESCRIPTION_NOTES, editDescription.text.toString())
